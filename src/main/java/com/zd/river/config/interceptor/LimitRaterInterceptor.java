package com.zd.river.config.interceptor;

import com.zd.river.common.annotation.RateLimiter;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Value("${river.rateLimit.enable}")
    private boolean rateLimitEnable;

    @Value("${river.rateLimit.limit}")
    private Integer limit;

    @Value("${river.rateLimit.timeout}")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (rateLimiter != null) {
            ...
            if (StrUtil.isBlank(token3)) {
                throw new RiverException("当前访问人数太多啦，请稍后再试");
            }
        }
        return true;
    }
